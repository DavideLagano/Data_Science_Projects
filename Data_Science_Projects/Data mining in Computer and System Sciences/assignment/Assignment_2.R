  ...
  while(TRUE){
    ...
    for(i in 1:nrow(x)){
      for(j in 1:nrow(centroids)){
        ...
        dist_from_centroids[i,j] <- my_dist_calculator(x[i,], centroids[j,],"euclidean")
      }
    }
...
for(k in 1:7){
  ...
  km.cl <- kmeans(cluster.data, centers = k, nstart = 1)
...
# In according to the Elbow method, the K value that I would choose is 3, because
# untill K=3 the clusters add much information, but from K=4 the marginal gain will
# drop. This is also explained by the "elbow criterion".
...
#I choose 0.4198 silhouette value because the coefficient
#must be as close as possible to 1 because it means that
#inside, the clusters are cohesive and also well separated
#one from the other.
...
hierarchial.clust <- hclust(distance_matrix, method = "average")
...
plot(small.dataset, col = clusters, pch = as.character(clusters), cex = 0.7, main="Hierarchical Cluster")
