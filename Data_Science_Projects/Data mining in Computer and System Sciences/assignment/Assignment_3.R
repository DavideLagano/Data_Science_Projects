...
load("C:/Users/david/Desktop/data mining/assignment 3/entropy_data.RData")
head(entropy.data)
...
getInfoGain <- function(split_var, class_var){
  ...
  for(i in 1:nrow(attr.class.freq)){
    ...
    # *** ATTENTION !!! *** #
    child.node.freq <- attr.class.freq[i,]/sum(class.freq)
    ...
    # *** ATTENTION !!! *** #
    entropy_split <- entropy_split + child.node.entropy*num_obs_child.node
  ...
  for(i in 1:number_of_columns){
    ...
    if(class(data.X[,i]) == "numeric"){
      ...
      for(j in 1:(length(x_sort_unique)-1)){
        ...
        # *** ATTENTION !!! *** #
        info_gain_for_this_split <- getInfoGain(data.Y, lessthan_cutoff)
...
load("C:/Users/david/Desktop/data mining/assignment 3/Perceptron_data.RData")
...
for(i in 1:length(cost.parameter)){
  ...
  for(j in 1:num.folds){
    ...
    # *** ATTENTION !!! *** #
    svm.model <- svm(formula = class ~ ., data = train.set, kernel = "radial", degree = 2, cost=cost.parameter[i], probability = TRUE)
...
# I will choose 1.0 cost parameter because is the value that give the best avg.accuracy, 0.835.
